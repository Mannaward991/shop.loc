...
-- Дамп структуры базы данных shop02
CREATE DATABASE IF NOT EXISTS `shop02` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `shop02`;

-- Дамп структуры для таблица shop02.goods
CREATE TABLE IF NOT EXISTS `goods` (
  ...
  UNIQUE KEY `goods_article_unique` (`goods_article`),
  KEY `goods_to_groups` (`goods_groups`),
  CONSTRAINT `goods_to_groups` FOREIGN KEY (`goods_groups`) REFERENCES `groups` (`groups_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы shop02.goods: ~36 rows (приблизительно)
...
CREATE TABLE IF NOT EXISTS `groups` (
  ...
  UNIQUE KEY `groups_id_unique` (`groups_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы shop02.groups: ~9 rows (приблизительно)
...
-- Дамп структуры для таблица shop02.news
CREATE TABLE IF NOT EXISTS `news` (
  `news_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Уникальный id новости.',
...
-- Дамп данных таблицы shop02.news: ~3 rows (приблизительно)
...
-- Дамп структуры для таблица shop02.orders
CREATE TABLE IF NOT EXISTS `orders` (
  `orders_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id заказа.',
  ...
  KEY `orders_confirmation` (`orders_confirmation`),
  KEY `orders_cookies_id` (`orders_cookies_id`)
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8;

-- Дамп данных таблицы shop02.orders: ~10 rows (приблизительно)
...
-- Дамп структуры для таблица shop02.orders_content
CREATE TABLE IF NOT EXISTS `orders_content` (
  ...
  UNIQUE KEY `os_id-article_unique` (`oc_id`,`oc_article`),
  KEY `oc_to_orders` (`oc_id`),
  KEY `oc_to_goods` (`oc_article`),
  CONSTRAINT `oc_to_goods` FOREIGN KEY (`oc_article`) REFERENCES `goods` (`goods_article`),
  CONSTRAINT `oc_to_orders` FOREIGN KEY (`oc_id`) REFERENCES `orders` (`orders_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы shop02.orders_content: ~14 rows (приблизительно)
/*!40000 ALTER TABLE `orders_content` DISABLE KEYS */;
INSERT INTO `orders_content` (`oc_id`, `oc_article`, `oc_quantity`) VALUES
	...
	(10, 100035, 4),
	(11, 100025, 1);
/*!40000 ALTER TABLE `orders_content` ENABLE KEYS */;

-- Дамп структуры для таблица shop02.price
CREATE TABLE IF NOT EXISTS `price` (
  ...
  UNIQUE KEY `price_article-date_unique` (`price_article`,`price_date`),
  CONSTRAINT `price_to_goods` FOREIGN KEY (`price_article`) REFERENCES `goods` (`goods_article`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Дамп данных таблицы shop02.price: ~38 rows (приблизительно)
...
-- Дамп структуры для таблица shop02.reviews
CREATE TABLE IF NOT EXISTS `reviews` (
  `reviews_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id коментария',
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='Таблица отзывов.';

-- Дамп данных таблицы shop02.reviews: ~3 rows (приблизительно)
